...
- **多源爬虫**：覆盖 16 个中巴信息源（1 个因 SSL/超时暂停），包含官方源与高可信非官方源，最多 12 线程并发抓取
- **三层 AI 筛选**：
  1. Gemini Flash Lite 200 并发粗筛 → 过滤无关文章
  2. Gemini Flash 逐篇独立评分（0-100）→ 精选前 N 篇（默认 6，可在 Web 端配置 1-10）
  3. Claude Sonnet 深度分析 → 生成专业情报简报
- **Web 界面**：浏览器操作，选择日期即可生成报告，支持进度追踪、耗时显示、任务终止和报告下载
- **断点续跑**：每阶段（爬虫/粗筛/深度评分）自动缓存中间结果，失败后可从断点恢复，无需重跑全流程
- **超时保护**：单个信息源抓取超过 5 分钟自动终止，防止个别源阻塞整条 pipeline
- **冷启动保活**：Render 部署自动 keep-alive，每 10 分钟 ping 自身 `/health` 端点及 Sonnet API 代理，避免冷启动延迟
...
┌──────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  爬虫层       │───▶│ 粗筛          │───▶│ 深度评分      │───▶│  报告生成     │
│ 12线程并发    │    │ Gemini Lite  │    │ Gemini Flash │    │ Claude Sonnet│
│              │    │ 200并发       │    │ 逐篇打分0-100│    │ 前N篇(可配置)│
└──────────────┘    └──────────────┘    └──────────────┘    └──────────────┘
    │                    │                    │                    │
    ▼                    ▼                    ▼                    ▼
  ~90篇             ~15篇相关          前N篇精选(1-10)      专业情报简报
```

## 新闻源列表

说明：
- `官方` 来源以政府、央行、统计机构等机构官网为主
- `非官方` 来源单独标注，并补充可信度，用于区分主流媒体、研究机构、行业协会和双边经贸机构
...
| 人民网国际频道 | 中方官方 | 中国 | ✅ |
| 新华网 | 中方官方 | 中国 | ✅ |
| 海关总署 | 中方官方 | 中国 | ⏸️ 504超时 |
| 港澳事务办公室 | 中方官方 | 中国 | ✅ |
| MDIC 新闻 | 巴西官方 | 巴西 | ✅ |
| Siscomex 新闻 | 巴西官方 | 巴西 | ✅ |
| Fazenda 新闻 | 巴西官方 | 巴西 | ✅ |
| IBGE 新闻 | 巴西官方 | 巴西 | ✅ (JSON API) |
| Planalto | 巴西官方 | 巴西 | ✅ |
| BCB Focus | 巴西官方 | 巴西 | ✅ |
| FGV IBRE 博客 | 巴西非官方-研究机构 | 巴西 | ✅ |
| CNI 新闻 | 巴西非官方-行业协会 | 巴西 | ✅ |
| CEBC 双边贸易 | 巴西非官方-双边经贸机构 | 巴西 | ✅ |
| DatamarNews | 巴西非官方-数据型媒体 | 巴西 | ✅ |
